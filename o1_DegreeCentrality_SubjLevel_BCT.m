...
if ~exist(sprintf('/mfip/mfip1/arielle/PhDProject2/data/%s/degree/%s%d', groupName, parcelName, parcelNum), 'dir')
    mkdir(sprintf('/mfip/mfip1/arielle/PhDProject2/data/%s/degree/%s%d/', groupName, parcelName, parcelNum))
end
...
for iSub = 1:size(info, 1)
    ...
    for ithresh = 1:length(thresh)
        ...
        writetable(T, sprintf('/mfip/mfip1/arielle/PhDProject2/data/%s/degree/%s%d/degreeResults_sub-%s_thresh-%f.csv', ...
