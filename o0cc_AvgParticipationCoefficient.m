...
for ithresh = 1:length(thresh)
  ...
  writetable(T, sprintf('/mfip/mfip1/arielle/PhDProject2/results/%s/participationCoefficient/%s%d_averageParticipationCoefficientResults_thresh-%f_group-%s.csv', ...
    ...
    for idx = 1:size(avgCorrMatrix, 1)
         ...
         toVisualize(idxBig) = participationCoefficient(idx);
    end
    ...
    savegifti(lh, rh, groupName, 'participationCoefficient', parcelName, parcelNum, thresh, subjType)
